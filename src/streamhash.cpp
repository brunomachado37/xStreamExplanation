...
namespace std {
  ...
  vector<float>
  streamhash_project(vector<string>& fields,
                     vector<uint64_t>& h, float density, float constant, vector<vector<bool>>& feature_projection_map) {
    uint k = h.size();
    vector<float> xp(k, 0.0);
    for (uint i = 0; i < k; i++) {
      for (uint j = 0; j < fields.size(); j++) {
        ...
        feature_value = atof(strtok(NULL, ":"));
        free(dup);
        feature_projection_map[i][j] = streamhash_hash(feature_idx, h[i], density, constant) != 0.0;
        xp[i] += (feature_value * streamhash_hash(feature_idx, h[i], density, constant));
      }
    }
    return xp;
